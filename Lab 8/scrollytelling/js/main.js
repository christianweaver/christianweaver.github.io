...
d3.csv('data/assignments.csv').then(_data => {
  ...
  const hardestAssignment = [...data].sort((a,b) => b.time - a.time)[0];
  d3.select('#max-time').text(hardestAssignment.time);
  d3.select('#max-assignment-time').text(hardestAssignment.time);
